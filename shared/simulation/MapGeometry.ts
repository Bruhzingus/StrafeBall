...
function pushBleacherEndCapCollisionBoxes(
  ...
  const railSegments = BLEACHER_LAYOUT.tierCount;
  ...
  for (let i = 0; i < railSegments; i += 1) {
    ...
    boxes.push(aabbFromCenter(
      innerX + side * runMid,
      runMid * slope + BLEACHER_ENDCAP_LAYOUT.railHeightAboveNosing,
      railZ,
      segRun * 0.5 + railRadius,
      segRise * 0.5 + railRadius,
      railRadius,
      { kind: 'bleacher', id: `bleacher_endcap_rail_top_${side}_${zSign}_${i}` }
    ));
  }
