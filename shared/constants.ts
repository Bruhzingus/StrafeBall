...
/** Combat lag-compensation windows. NOT rate-independent tuning — derived from net timing. */
export interface CombatTiming {
  catchStartupMs: number;
  catchActiveMs: number;
  catchCooldownMs: number;
  catchDefaultRttMs: number;
  catchMaxRttMs: number;
  catchRewindMs: number;
  catchHitGraceMs: number;
  defenseMaxRewindMs: number;
  defenseInputGraceMs: number;
  defenseHistoryMs: number;
}
 ...
 */
export function deriveCombatTimingConstants(timing: {
  serverStepMs: number;
  ...
  const defenseHistoryMs = Math.ceil(
    Math.max(700, catchMaxRewindMs + CATCH_ACTIVE_MS + catchInputGraceMs + 100)
  );
  return {
    catchStartupMs: 0,
