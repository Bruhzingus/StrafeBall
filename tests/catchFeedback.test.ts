...
describe('catch recoil feedback', () => {
  it('does not recoil for ordinary quick throws', () => {
    ...
    expect(catchRecoilForVelocity({ x: 35.99, y: 0, z: 0 })).toBeNull();
  });

  it('recoils in the incoming ball direction, continuing away from the thrower', () => {
    const recoil = catchRecoilForVelocity({ x: 0, y: 0, z: -42 });
  ...
  it('scales only the hard-throw range from minimum to maximum recoil', () => {
    const minimum = catchRecoilForVelocity({ x: 36, y: 0, z: 0 });
    const middle = catchRecoilForVelocity({ x: 39, y: 0, z: 0 });
    const maximum = catchRecoilForVelocity({ x: 48, y: 0, z: 0 });
