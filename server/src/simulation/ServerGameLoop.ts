...
const BACKFLIP_QTE_ELIGIBILITY_SECONDS =
  GAME_CONSTANTS.backflip.qte.armDelaySeconds +
  GAME_CONSTANTS.backflip.qte.durationSeconds +
  0.35;
const BACKFLIP_QTE_FALLBACK_SECONDS =
  (2 * GAME_CONSTANTS.backflip.verticalImpulse) / GAME_CONSTANTS.ball.gravity +
...
export class ServerGameLoop {
  ...
  private readonly backflipLandingPendingByPlayerId = new Set<string>();
  private readonly backflipQteSecondsByPlayerId = new Map<string, number>();
  ...
  private readonly mapEffects = new MapEffectSystem();
  ...
  get mapEffectSystem(): MapEffectSystem { return this.mapEffects; }
  ...
  handlePickup(playerId: string): ActionResult {
    ...
    if (armor) return { ok: true, log: `pickup accepted player=${playerId} ball=${armor.ballId} hand=${armor.hand} (armor)` };
  ...
  handleThrow(playerId: string, request: ThrowRequestPayload): ActionResult {
    ...
    if (ball.kind === 'cannon' && handState.mode !== 'charging') return { ok: false, reason: 'charge-required' };
    ...
    const recentBackflipFallback = player.movementInternal.backflipCooldown > Math.max(
      0,
      GAME_CONSTANTS.backflip.cooldownSeconds - BACKFLIP_QTE_FALLBACK_SECONDS
    );
    ...
    if (ball.kind === 'cannon') {
      throwCalc.velocity = cannonLaunchVelocity(forward, charge01);
      throwCalc.curveAccel = vec3(); throwCalc.dropScale = cannonDropScale(charge01); throwCalc.isSuper = false;
    } else if (ball.kind === 'bomb') {
      throwCalc.velocity = scale(throwCalc.velocity, GAME_CONSTANTS.powerup.bombThrowSpeedMultiplier);
    }
  ...
  private updateBackflipQteEligibility(player: PlayerState, dt: number, wasBackflipActive: boolean): void {
    const remaining = this.backflipQteSecondsByPlayerId.get(player.id) ?? 0;
    if (remaining > dt) this.backflipQteSecondsByPlayerId.set(player.id, remaining - dt);
    else if (remaining > 0) this.backflipQteSecondsByPlayerId.delete(player.id);
    ...
    if (
      this.backflipLandingPendingByPlayerId.has(player.id) &&
  ...
  advance(): void {
    ...
    if (powerupsActive) {
      this.powerupSystem.beforeBalls(this.state, fixedDt, this.matchSettings.livesPerPlayer,
        (spawnIndex, position) => this.mapEffects.tryStart(this.state, spawnIndex, position));
      this.mapEffects.step(this.state, fixedDt, {
        damage: (player) => { if (active) this.applyEnvironmentDamage(player.id); },
    ...
    if (powerupsActive && this.state.mapEffect?.kind !== 'lava') this.respawnCrowdedBalls(fixedDt);
    if (powerupsActive) {
      ...
      this.powerupSystem.afterBalls(this.state, fixedDt, (ball, target) => {
        ...
        if (!active) return; // lobby bombs are fireworks, not damage
  ...
  private updatePlayer(player: PlayerState, dt: number, input: PlayerInput, seq: number): void {
    ...
    const result = stepMovement(
      ...
      mapEffectGravityScale(this.state.mapEffect)
    );
    player.movement = result.movement;
    if (this.state.match.status !== 'playing' && this.state.match.status !== 'warmup') result.internal.buffs = player.movementInternal.buffs;
  ...
  private handleInputThrows(playerId: string, input: PlayerInput): void {
    ...
    for (const hand of ['left', 'right'] as const) {
      ...
      if (ball?.kind === 'heal' && (hand === 'left' ? input.leftHandPressed : input.rightHandPressed)) this.handleThrow(playerId, { hand });
    }
   ...
   */
  private updateBalls(dt: number, hitActive: boolean, defenseActive: boolean): void {
    ...
    const ballConstants = ballConstantsForEffect(this.state.mapEffect);
    ...
    for (const ballId in this.state.balls) {
      ...
      for (let sub = 0; sub < LIVE_BALL_COMBAT_SUBSTEPS && !combatDone; sub++) {
        ...
        const advanced = advanceBall(current, subDt, ballConstants);
        let resolved = advanced;

        if (isGrenadeKind(resolved.kind)) {
          // Grenades stick to the first player they cross (never the thrower on release).
          const target = resolved.curveDistance >= GAME_CONSTANTS.powerup.grenadeStickMinDistance
        ...
        } else if (defenseActive && isBallCatchableInFlight(resolved)) {
  ...
  private tryHit(ball: BallState, segPrev: Vec3, segCurr: Vec3): BallState | null {
    ...
    for (const targetId in this.state.players) {
      ...
      if (ball.kind === 'cannon') {
        ...
        if (targetId === ownerId && ball.curveDistance < GAME_CONSTANTS.powerup.cannonSelfHitMinDistance) continue;
      } else {
  ...
  private applyEnvironmentDamage(playerId: string): void {
   ...
   */
  private knockOverMatsNear(position: Vec3, radius: number): void {
    let knockedAny = false;
    for (const spec of this.activeMatSpecs) {
      if (this.knockedOverMatIds.has(spec.id)) continue;
      const box = matCollisionBox(spec);
      const dx = position.x - clamp(position.x, box.minX, box.maxX);
      const dz = position.z - clamp(position.z, box.minZ, box.maxZ);
      if (dx * dx + dz * dz > radius * radius) continue;
      const away = normalize(vec3(spec.x - position.x, 0, spec.z - position.z), vec3(0, 0, 1));
      const fallDir = matFallDirection(away);
      this.state.mats[spec.id] = { ...this.state.mats[spec.id], knockedOver: true, knockDirection: vec3(fallDir.x, 0, fallDir.z) };
      this.knockedOverMatIds.add(spec.id);
      knockedAny = true;
    }
    if (knockedAny) this.rebuildCollisionBoxes();
  }

  /** First living player (other than the thrower) whose body the grenade's swept path crosses. */
  private findGrenadeTarget(ball: BallState, segPrev: Vec3, segCurr: Vec3): PlayerState | null {
    const radius = playerBallHitRadius();
    for (const id in this.state.players) {
      const target = this.state.players[id];
      if (id === ball.ownerId || !this.isPlayerAlive(target)) continue;
      const hitbox = playerHitCapsule(target);
      if (sweptBallHitsBody(segPrev, segCurr, hitbox.base, hitbox.top, radius)) return target;
    }
    return null;
  }
...
const UNLIMITED_BOUNCE_RULE: BounceRule = {
